...
if __name__=="__main__":
    ...
    # Access masks
    mask_poly_list = glob.glob(os.path.join(MASK_POLY_DIR, '*'))
    mask_poly_list.sort()
    
   
    mask_line_list = glob.glob(os.path.join(MASK_LINE_DIR, '*'))
    mask_line_list.sort()
    
    image_mask_check(image_list,mask_poly_list)
    image_mask_check(image_list,mask_line_list)
    
    # Check image-mask match
    if image_mask_check(image_list, mask_poly_list):
        ...
        batch_polygon_list=mask_poly_list[:BATCH_SIZE]
        batch_polygon_tensor=tensorize_mask(batch_polygon_list,output_shape,2)#call function
        
        batch_line_list=mask_line_list[:BATCH_SIZE]
        ...
        # Check
        print("For labels:\ndtype is "+str(batch_line_tensor.dtype))
        print("Type is "+str(type(batch_line_tensor)))
        print("The size should be ["+str(BATCH_SIZE)+", 3, "+str(HEIGHT)+", "+str(WIDTH)+"]")
        print("Size is "+str(batch_line_tensor.shape))